...
void systems::Spawn::Clouds() {
  ...
  if (count == 0) {
    // First cloud
    entities::background::CreateCloud(registry_, game_->res_manager(), next_cloud_spawn_x);
    ++count;

    // Spawn up to kMaxCount clouds with random spacing
    while (count < kMaxCount) {
      std::uniform_real_distribution<double> dist(kCloudMinSpacing, kCloudMaxSpacing);
  ...
  kCloudView.each([&](auto entity, const auto& kTransform) {
    if (kTransform.position.x <= -kTransform.position.w) {
      ...
      std::uniform_real_distribution<double> dist(kCloudMinSpacing, kCloudMaxSpacing);
      double spacing = dist(GetRNG());
      const auto kPos = kTransform.position.x + kTransform.position.w + spacing + kBounds.position.w;
